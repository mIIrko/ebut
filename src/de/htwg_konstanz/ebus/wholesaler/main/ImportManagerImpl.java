...
public class ImportManagerImpl implements IImportManager {
                        ...
                        if (articleContent.item(j).getNodeName().equals("SUPPLIER_AID")) {
                            ...
                            for (int k = 0; k < list.getLength(); k++) {
                                ...
                                if (list.item(k).getNodeType() == Node.ELEMENT_NODE) {
                                    ...
                                    purchasePrices.addAll(processPurchasePrices(boProduct, articlePrice));
                                }
                            }
                        }
     ...
     * BOPurchasePrice
     *
     * @param prod:         the involved BOProduct instance
     * @param articlePrice: the involved price node
     ...
     * @return a BOPurchasePrice instance
     */
    public List<BOPurchasePrice> processPurchasePrices(BOProduct prod, Node articlePrice) {
        PriceContainer container = processPrice(articlePrice);
        List<BOPurchasePrice> purchasePrices = new ArrayList<>();
        // process all territories
        List<String> isocodes = container.getTerritoryList();
     ...
     * BOSalesPrice
     *
     * @param purchasePrice: the involved purchase price
